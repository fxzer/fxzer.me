...
`git fetch -p` （或者 `--prune`）命令用于从远程仓库中拉取最新的提交记录，同时也会删除 _本地不存在的远程分支_；不会更新本地分支和远程分支之间的关联关系，需要手动执行 `git merge` 或 `git rebase` 命令，或者使用 `git pull` 命令。例：删除远程 `dev`分支后，使用此命令，本地的远程分支也会被清理。
